...
def generate_text_from_model(textlen = 200):
    ...
    logger.debug("Generating text (num_chars = %s)...", textlen)
    starting_word = "The "
    input_x = [vocab_to_idx[i] for i in starting_word]
...
def predict(saved_params, num_classes, input_x, textlen=200, isEstimator=False, predict_configs=None):
    def sampleCharFromPrediction(preds):
        """
        Samples a character based on prediction.
        """
    ...
    def pickRandomTopNCharFromPrediction(preds, topn, vocabsz):
        """
        Picks one of the topn characters predicted,
        uniformly randomly.
        """
    ...
    model = CharacterRNN(num_classes, saved_params).build_computation_graph()
    ...
    with tf.Session() as sess:
        ...
        # Warm up our model with the seed word
        for c in input_x[:-1]:
            x = np.array([[c]])
            if init_state is not None:
                ...
                feed_dict = {model['x']: x}
            _, init_state = sess.run([predictions, model['final_state']], feed_dict)
        lastgenchar = input_x[-1]
        ...
        for i in range(textlen):
            x = np.array([[lastgenchar]])
            feed_dict = {model['x']: x, model['init_state']: init_state}
            preds, init_state = sess.run([predictions, model['final_state']], feed_dict)
            # lastgenchar = sampleCharFromPrediction(preds)
            lastgenchar = pickRandomTopNCharFromPrediction(preds, 5, num_classes)
            predict_idx.append(lastgenchar)
    return predict_idx
...
def compute_average_line_loss(local_filename, anomaly_percentile=95, input_url=None):
    ...
    character is summed up and divided by the number of characters to compute the
    ...
    model = CharacterRNN(saved_num_classes, saved_params).build_computation_graph()
    avglinelosses = []
    ...
    with tf.Session() as sess:
        ...
        for lbi in linebreak_indices:
            ...
            if lbi - prev_lbi < 5:
                prev_lbi = lbi
                continue
            #Go thro' each char of each line
            for ci in range(prev_lbi, lbi):
                if ci + 1 >= len(test_dp.corpus):
                    break
                x = np.array([[test_dp.corpus[ci]]])
                y = np.array([[test_dp.corpus[ci + 1]]])
                if init_state is not None:
